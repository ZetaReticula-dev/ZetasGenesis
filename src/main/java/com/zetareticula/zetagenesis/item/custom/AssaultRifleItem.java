...
public class AssaultRifleItem extends Item {
    ...
    public AssaultRifleItem(Settings settings) {super(settings);}

    @Override
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        {
            if (world.isClient) {   return TypedActionResult.pass(user.getStackInHand(hand)); } {
            world.playSound(null, user.getX(), user.getY(), user.getZ(), GenesisSounds.MA5B_FIRE, SoundCategory.BLOCKS, 10.0F, 1.0F);
