...
public class SkilletBlockEntity extends BlockEntity implements HeatableBlockEntity, Clearable {
    ...
    private final int[] cookingTotalTime = new int[1];
    ...
    public SkilletBlockEntity(BlockPos pos, BlockState state) {
        super(GenesisBlockEntity.SKILLET_BLOCK_ENTITY, pos, state);
    }

    public static void litServerTick(World world, BlockPos pos, BlockState state, SkilletBlockEntity skillet) {
        boolean bl = false;

        for (int i = 0; i < skillet.itemsBeingCooked.size(); i++) {
            ItemStack itemStack = skillet.itemsBeingCooked.get(i);
            if (!itemStack.isEmpty()) {
                bl = true;
                skillet.cookingTime[i]++;
                if (skillet.cookingTime[i] >= skillet.cookingTotalTime[i]) {
                    SingleStackRecipeInput singleStackRecipeInput = new SingleStackRecipeInput(itemStack);
                    ItemStack itemStack2 = (ItemStack)skillet.matchGetter
                    ...
                    if (itemStack2.isItemEnabled(world.getEnabledFeatures())) {
                        ...
                        skillet.itemsBeingCooked.set(i, ItemStack.EMPTY);
    ...
    public static void unlitServerTick(World world, BlockPos pos, BlockState state, SkilletBlockEntity skillet) {
        boolean bl = false;

        for (int i = 0; i < skillet.getItemsBeingCooked().size(); i++) {
            if (skillet.cookingTime[i] > 0) {
                bl = true;
                skillet.cookingTime[i] = MathHelper.clamp(skillet.cookingTime[i] - 2, 0, skillet.cookingTotalTime[i]);
            }
        }
    ...
    public static void clientTick(World world, BlockPos pos, BlockState state, SkilletBlockEntity skilletBlockEntity) {
        Random random = world.random;

        int i = ((Direction)state.get(StoveBlock.FACING)).getHorizontal();

        for (int j = 0; j < skilletBlockEntity.itemsBeingCooked.size(); j++) {
            if (!skilletBlockEntity.itemsBeingCooked.get(j).isEmpty() && random.nextFloat() < 0.2F) {
